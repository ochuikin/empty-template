...
public class WordFetcher {
    // Не надо так делать, даже с контекстом Application. Качайте в сервисе, например. Но помните, что он тоже может
    // заработать на главном потоке, посмотрите IntentService. А вообще доступно огромное количество официальных
    // и сторонних решений, поресерчите проблему и подумайте, что больше подойдет в какой ситуации.

    // И забудьте про статику в андроиде (на самом деле нет). Просто помните, что статика не привязана к жизненным циклами
    // андроида, а забить на них почти никогда нельзя, если хотите добиться, чтобы приложения корректно работало
    // в большинстве ситуаций. Можно представлять работу асинхронной операции на линейке времени, где располагать
    // условно все события жизни UI/приложения, и думать, что будет, если операция вернется в любой из точек.
    ...
    private static class AsyncLoad extends AsyncTask<Context, Void, Void> {
        ...
        protected Void doInBackground(Context... context) {
            ...
            try {
                ...
                // Константы надо выносить сходу, потом будет уже поздно
